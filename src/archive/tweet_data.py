...
def get_tweet(
    ...
    # Collect tweets
    tweets = tw.Cursor(
        api.search, q=search_words, lang="en", until=date_until, since=date_since
    ).items(tweet_number)

    tweets_list = [
        [tweet.created_at, tweet.text, tweet.user.screen_name, tweet.user.location]
        for tweet in tweets
    ]
    tweets_data = pd.DataFrame(
        data=tweets_list, columns=["created_time", "tweet_text", "user", "location"]
    )
